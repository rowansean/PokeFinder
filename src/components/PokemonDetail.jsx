...
export default function PokemonDetail({ featuredPokemon }) {
  ...
  async function fetchPokemonDetails() {
    const res = await fetch(
      `https://pokeapi.co/api/v2/pokemon/${featuredPokemon.name}`
    );
  ...
  async function playCry() {
    const cry = new Audio(
      `https://raw.githubusercontent.com/PokeAPI/cries/main/cries/pokemon/latest/${pokemonDetails.id}.ogg`
    );
    cry.play();
  }
  ...
  return (
    <div className="flex flex-col max-h-full items-center gap-4">
      ...
      <Card className="bg-[#d13a3ad9] w-80 h-72 border-none p-4 shadow-inner overflow overflow-y-auto">
        <CardTitle className="text-white capitalize m-1 flex gap-2">
          {pokemonDetails.name}
        ...
        {pokemonDetails.types.map((type, index) => (
          <Badge
            ...
            } text-black text-xs capitalize m-1`}
          >
            {type.type.name}
          </Badge>
        ))}
